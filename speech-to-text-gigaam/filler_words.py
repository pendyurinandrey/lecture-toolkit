"""
Удаление слов-паразитов из уже пунктуированного вывода GigaAM.

В отличие от speech-to-text-whisper (там пунктуация ещё не расставлена на
момент удаления паразитов), здесь каждое слово уже может нести пунктуацию
("вот,", "«ну", "ж." и т.п.) — поэтому при удалении слова-паразита его
пунктуация не выбрасывается вместе с ним:
...
Список слов намеренно консервативный — см. speech-to-text-whisper/filler_words.py
за тем же рассуждением про многозначные слова ("значит", "просто" и т.п.).
"""
